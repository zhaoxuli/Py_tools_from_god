...
class RandCropper(RandSampler):
    ...
    def sample(self, label):
        ...
        for trial in range(self.max_trials):
            ...
            valid_mask = np.where(label[:, 4] > -1)[0]
            gt = label[valid_mask, :]
            ...
            if ious is not None:
                ...
                for i in range(valid_mask.size):
                    if ious[i] > 0:
                        xmin = max(0., (gt[i, 0] - l) / new_width)
                        ymin = max(0., (gt[i, 1] - t) / new_height)
                        xmax = min(1., (gt[i, 2] - l) / new_width)
                        ymax = min(1., (gt[i, 3] - t) / new_height)
                        new_gt_boxes.append([xmin, ymin, xmax, ymax, gt[i, 4]])
                if not new_gt_boxes:
                    continue
    ...
    def _check_satisfy(self, rand_box, gt_boxes):
        ...
        rs = np.ones(num_gt) * r
        bs = np.ones(num_gt) * b
        mask = np.where(ls < gt_boxes[:, 0])[0]
        ls[mask] = gt_boxes[mask, 0]
        mask = np.where(ts < gt_boxes[:, 1])[0]
        ts[mask] = gt_boxes[mask, 1]
        mask = np.where(rs > gt_boxes[:, 2])[0]
        rs[mask] = gt_boxes[mask, 2]
        mask = np.where(bs > gt_boxes[:, 3])[0]
        bs[mask] = gt_boxes[mask, 3]
        w = rs - ls
        w[w < 0] = 0
        h = bs - ts
        h[h < 0] = 0
        ...
        union_area += (gt_boxes[:, 2] - gt_boxes[:, 0]) * (gt_boxes[:, 3] - gt_boxes[:, 1])
        ...
        if self.config['gt_constraint'] == 'center':
            for i in range(ious.shape[0]):
                if ious[i] > 0:
                    gt_x = (gt_boxes[i, 0] + gt_boxes[i, 2]) / 2.0
                    gt_y = (gt_boxes[i, 1] + gt_boxes[i, 3]) / 2.0
                    if gt_x < l or gt_x > r or gt_y < t or gt_y > b:
                        return None
        elif self.config['gt_constraint'] == 'corner':
            for i in range(ious.shape[0]):
                if ious[i] > 0:
                    if gt_boxes[i, 0] < l or gt_boxes[i, 2] > r \
                        or gt_boxes[i, 1] < t or gt_boxes[i, 3] > b:
                        return None
        return ious
